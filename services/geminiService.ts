...
- 反派必须嘴贱、招恨、作死；主角必须当场回敬，不隔夜。`
  : `【流派：非情绪流（脑洞热梗引擎）】
文风诙谐幽默，融入网络、影视热梗，快节奏，脑洞要大，每一集都要有小卡点。
...
- 反派每集至少一次“作死加码”，主角当场“加倍奉还”。`;
      ...
      const continuityInstruction = isFirstBatch
        ? `【爽点路线确认（必须在第1-2集完成）】：
本剧必须在开篇明确建立“稳定爽点模式”，让观众清楚知道本剧主打哪一类爽。

要求：
1. 在第一集中，至少完整示范一次“本剧核心爽点模式”（冲突 → 误判 → 主角优势 → 当众反转）。
2. 在结尾或角色反应中，必须明确暗示：类似爽点还会反复出现，而非一次性事件。
3. 主角的胜利方式必须稳定一致（例如：永远赢在信息差/系统/身份错位），不得频繁更换。
...
补充：
1. 若含穿越/重生：必须先用“离谱死法/憋屈处境”秒切，再无缝落到当前冲突现场。
2. 主角第一句必须立人设：吐槽/狠话/反击三选一，禁止平淡叙述。
3. 设定只能用“宣告式台词/系统播报”交代，禁止解释型长段落。`
  : `【硬核无缝衔接指令（硬性）】：
...
【人物极端化铁律（最高优先级）】：
本剧所有核心人物必须“功能化极端”，而非写实。

- 圣母角色：必须持续做出“道德正确但必然伤害主角”的选择，且自我感觉高尚，从不反省。
- 绿茶角色：所有伤害行为必须伪装成善意、无辜或误会，嘴上为别人好，结果最毒。
- 反派角色：坏得直接、坏得嚣张、坏得理直气壮，不得洗白，不得共情。
- 任何人物一旦出现“中立、摇摆、反思”，立刻通过事件逼其站队。

【判定标准】：
观众必须在角色出场3次内，对其产生明确情绪标签：
“这个人欠收拾 / 这个人迟早翻车 / 这个人一出事我就爽。
【单集结构硬模板（必须执行）】：
