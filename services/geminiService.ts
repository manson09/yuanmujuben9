  ...
  generateScriptBatch: async (
    ...
    return callWithRetry(async () => {
      ...
      const styleInstruction = scriptStyle === '情绪流' 
        ? `【流派：情绪流（适配女频逻辑）】极致冲突，节奏爆破，反派嚣张化，情绪拉扯拉满。
           女频核心爽点：1.极致手撕绿茶；2.身份马甲掉落；3.极致团宠/被守护；4.追妻火葬场；5.宿命心动感。`
        : `【流派：非情绪流（适配男频逻辑）】诙谐幽默，反套路脑洞，对话有机锋。
           男频10大爽点：1.身份打脸；2.实力碾压；3.护短暴走；4.以牙还牙；5.隐藏大佬；6.财富碾压；7.医术/绝技震撼；8.美女倒追；9.绝境翻盘；10.群体下跪。`;

      // 原有逻辑：衔接指令
      const continuityInstruction = isFirstBatch
        ? `【开篇建模指令】：
          1. 3句内必须进入冲突。
          2. 若原著含穿越/重生，首集开头需用“闪回/极速剪辑感”描写原本离谱的死法或处境，随后瞬间切换到当前世界的冲突现场。
          3. 用主角对新身体/新环境的“第一吐槽”或“瞬间反击”快速立住人设。
          4. 绝不长篇大论解释世界观，通过正在发生的矛盾（如被退婚、被嘲讽）带出设定。`
        : `【硬核衔接指令】：
...
3. **状态校验**：严格检查[前序剧集结尾]中在场的人物名单，确保第一场戏不会凭空多出或减少人。`;
      ...
      // ========================================================
      // 核心补充：原本代码里未涵盖的“改编核心五步走”及“三大规范”
      // ========================================================
      const coreMethodology = `
【改编核心五步走（实操优先级）】：
1. 提炼爽点主轴：明确主角弧光、反杀节点、情绪高潮、金句爆点。适配漫剧十几秒耐心阈值。
2. 重建单集单点叙事：每集只设一个核心目标，8秒内看懂“本集期待”。
3. 人物动机直给化：第一集亮明角色性格。${scriptStyle === '情绪流' ? '强化情绪代入点' : '突出升级驱动'}。
4. 视觉化转译：内心独白、心理转折转化为具象画面（表情特写、反差镜头、符号化物件）。
5. 支线筛减合并：只保留推爽点或推情绪的必要支线。

【创作三大核心规范】：
- 剧情与节奏：主线单一聚焦，冲突依托反派关系链，拒绝“嘴炮”，装逼打脸结合燃炸场面。
- 人设对白：人设一致，反派坏得彻底，男主杜绝圣母心。台词有趣、有梗、服务于剧情。
- 结构设定：简化人物网，金手指设定单一清晰，重要场景细化描写突出冲击力。`;
      ...
      const response = await requestOpenRouter(
        ...
        [原著素材]：\n${novelText}`,
        {
          ...
          properties: {
            ...
            adaptationSummary: {
              type: "string", 
              description: "简要总结本批次对原著角色、剧情做的重大删改（如：删除了某人，合并了某情节），以及使用的爽点等级，以便维持一致性。"
            }
          },
